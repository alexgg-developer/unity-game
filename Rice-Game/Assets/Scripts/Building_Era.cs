...
public class Building_Era: Building
{
	...
	STATE _state;
    private bool b_IsProductionStopped = false;

    public Building_Era (JSONNode data, BUILDINGS type): base(data, type)
	{
	...
	public void sendRice(uint riceSent) 
	{
		...
		Debug.Log ("Building_Era: recibidos "+riceRecibed+" kg de "+riceSent+" Kg");
	...
	override public void newDayCallback()
	{
        if (!b_IsProductionStopped) {
			...
			//Debug.Log ("Building_Era: filledRate=" + filledRate + " (" + newState + ")");
        }
	}
	...
	*/
    ...
    public override List<Pair<string, string>> getSpecificBuildingInfo()
    {
        ...
        m_specificBuildingInfo.Add(new Pair<string, string>(Dictionary.getString("RICE_TO_DRY_AND_CLEAN") + " ", riceUnpreparedInfo + " " + measurementUnprepared));
        m_specificBuildingInfo.Add(new Pair<string, string>(Dictionary.getString("RICE_READY_TO_PACK"), ricePreparedInfo + " " + measurementPrepared));
